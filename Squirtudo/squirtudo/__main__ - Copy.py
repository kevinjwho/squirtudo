...
async def _maybe(channel, author, count, party):
    ...
    if count == 1:
        await Squirtudo.send_message(channel, _("{member} is interested!").format(member=author.mention))
    else:
...
async def _coming(channel, author, count, party):
    ...
    if not party:
        ...
        party = [allblue, allred, allyellow, allunknown]
    if count == 1:
        await Squirtudo.send_message(channel, _("{member} is on the way!").format(member=author.mention))
    else:
...
async def _here(channel, author, count, party):
    ...
    if not party:
        ...
        party = [allblue, allred, allyellow, allunknown]
    if count == 1:
        await Squirtudo.send_message(channel, _("{member} is at the raid!"+lobbymsg).format(member=author.mention))
    else:
...
async def _lobby(message, count):
    ...
    trainer_dict = server_dict[message.server.id]['raidchannel_dict'][message.channel.id]['trainer_dict']
    if count == 1:
        await Squirtudo.send_message(message.channel, _("{member} is entering the lobby!").format(member=message.author.mention))
    else:
        await Squirtudo.send_message(message.channel, _("{member} is entering the lobby with a total of {trainer_count} trainers!").format(member=message.author.mention, trainer_count=count))
    # Add trainer name to trainer list
