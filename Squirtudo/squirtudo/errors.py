...
def custom_error_handling(bot,logger):
    ...
    async def on_command_error(error, ctx):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            msg = "Team Management is not enabled on this server. **!{cmd_name}** is unable to be used.".format(cmd_name=ctx.command.name)
            await bot.send_message(ctx.message.channel,msg)
            pass

        elif isinstance(error, WantSetCheckFail):
            msg = "Pokemon Management is not enabled on this server. **!{cmd_name}** is unable to be used.".format(cmd_name=ctx.command.name)
            await bot.send_message(ctx.message.channel,msg)
            pass

        elif isinstance(error, WildSetCheckFail):
            msg = "Wild Reporting is not enabled on this server. **!{cmd_name}** is unable to be used.".format(cmd_name=ctx.command.name)
            await bot.send_message(ctx.message.channel,msg)
            pass

        elif isinstance(error, RaidSetCheckFail):
            msg = "Raid Management is not enabled on this server. **!{cmd_name}** is unable to be used.".format(cmd_name=ctx.command.name)
            await bot.send_message(ctx.message.channel,msg)
            pass

        elif isinstance(error, CityChannelCheckFail):
            server = ctx.message.server
            msg = "Please use **!{cmd_name}** in ".format(cmd_name=ctx.command.name)
            ...
            server = ctx.message.server
            msg = "Please use **!{cmd_name}** in the following channel".format(cmd_name=ctx.command.name)
            ...
            server = ctx.message.server
            msg = "Please use **!{cmd_name}** in a Raid channel. Use **!list** in any ".format(cmd_name=ctx.command.name)
            city_channels = bot.server_dict[server.id]['city_channels']
            if len(city_channels) > 10:
            ...
            server = ctx.message.server
            msg = "Please use **!{cmd_name}** in an Egg channel. Use **!list** in any ".format(cmd_name=ctx.command.name)
            city_channels = bot.server_dict[server.id]['city_channels']
            if len(city_channels) > 10:
                msg+="Region report channel to see active raids."
            else:
            ...
            msg = "**!{cmd_name}** can't be used in a Raid channel.".format(cmd_name=ctx.command.name)
            await bot.send_message(ctx.message.channel,msg)
            pass

        elif isinstance(error, ActiveRaidChannelCheckFail):
            server = ctx.message.server
            msg = "Please use **!{cmd_name}** in an Active Raid channel. Use **!list** in any ".format(cmd_name=ctx.command.name)
            city_channels = bot.server_dict[server.id]['city_channels']
            if len(city_channels) > 10:
                msg+="Region report channel to see active raids."
            else:
            ...
            server = ctx.message.server
            msg = "Please use **!{cmd_name}** in either a Raid channel or ".format(cmd_name=ctx.command.name)
            city_channels = bot.server_dict[server.id]['city_channels']
            if len(city_channels) > 10:
                msg+="a Region report channel."
            else:
            ...
            server = ctx.message.server
            msg = "Please use **!{cmd_name}** in either a Raid Egg channel or ".format(cmd_name=ctx.command.name)
            city_channels = bot.server_dict[server.id]['city_channels']
            if len(city_channels) > 10:
                msg+="a Region report channel."
            else:
            ...
            server = ctx.message.server
            msg = "Please use **!{cmd_name}** in either a EX Raid channel or one of the following region channels:".format(cmd_name=ctx.command.name)
            city_channels = bot.server_dict[server.id]['city_channels']
            if len(city_channels) > 10:
                msg+="a Region report channel."
            else:
            ...
            server = ctx.message.server
            msg = "Please use **!{cmd_name}** in a EX Raid channel. Use **!list** in any of the following region channels to see active raids:".format(cmd_name=ctx.command.name)
            city_channels = bot.server_dict[server.id]['city_channels']
            if len(city_channels) > 10:
                msg+="a Region report channel."
            else:
