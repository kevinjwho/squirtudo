...
async def ask(message, destination, user_id, *, react_list=['✅', '❎']):
    if isinstance(message, discord.Embed):
        msg = await Squirtudo.send_message(destination, embed=message)
    else:
        msg = await Squirtudo.send_message(destination, message)
    ...
    for r in react_list:
        ...
        await Squirtudo.add_reaction(msg, r)
    res = await Squirtudo.wait_for_reaction(react_list, message=msg, check=check, timeout=60)
...
async def template(ctx, *, sample_message):
    ...
    if errors:
        if msg.startswith("[") and msg.endswith("]"):
            ...
            await Squirtudo.send_message(ctx.message.channel, embed=embed)
        else:
        ...
        if msg.startswith("[") and msg.endswith("]"):
            await Squirtudo.send_message(ctx.message.channel, embed=discord.Embed(
                colour=ctx.message.server.me.colour, description=msg[1:-1].format(user=ctx.message.author.mention)))
        else:
            await Squirtudo.send_message(ctx.message.channel, msg.format(user=ctx.message.author.mention))
...
async def expiry_check(channel):
    ...
    global active_raids
    channel = Squirtudo.get_channel(channel.id)
...
async def on_member_join(member):
    ...
    if server_dict[server.id]['welcomechan'] == "dm":
        if welcomemessage.startswith("[") and welcomemessage.endswith("]"):
            await Squirtudo.send_message(member, embed=discord.Embed(colour=server.me.colour, description=welcomemessage[1:-1].format(server=server.name, user=member.mention)))
        else:
            await Squirtudo.send_message(member, welcomemessage.format(server=server.name, user=member.mention))
    else:
        default = discord.utils.get(server.channels, name = server_dict[server.id]['welcomechan'])
        if default:
            if welcomemessage.startswith("[") and welcomemessage.endswith("]"):
                await Squirtudo.send_message(default, embed=discord.Embed(colour=server.me.colour, description=welcomemessage[1:-1].format(server=server.name, user=member.mention)))
            else:
                await Squirtudo.send_message(default, welcomemessage.format(server=server.name, user=member.mention))
...
async def configure(ctx):
    ...
    try:
        await Squirtudo.delete_message(ctx.message)
            ...
            if teamreply.content.lower() == "y":
                ...
                for team in config['team_dict'].keys():
                    if team.lower() not in lowercase_roles:
                        try:
                            temp_role = await Squirtudo.create_role(server=server, name=team.lower(), hoist = False, mentionable = True)
                            server_roles.append(team.lower())
                        except discord.errors.HTTPException:
                            await Squirtudo.send_message(owner, "Maximum guild roles reached.")
                            return
                await Squirtudo.send_message(owner, embed=discord.Embed(colour=discord.Colour.green(), description="Team Assignments enabled! I've made some roles for you if you didn't have them, but please ensure you only have one role per team."))
                break
    ...
    if configcancel == False and (firstconfig == True or configgoto == "all" or configgoto == "welcome"):
        ...
        while True:
            ...
            if welcomereply.content.lower() == "y":
                ...
                await Squirtudo.send_message(owner, embed=discord.Embed(
                    ...
                    description=("Would you like a custom welcome message? "
                                 ...
                                 "Surround your message with [] to send it as an embed. **Warning:** Mentions within embeds may be broken on mobile, this is a Discord bug.")).set_author(name="Welcome Message", icon_url=Squirtudo.user.avatar_url))
                while True:
                    welcomemsgreply = await Squirtudo.wait_for_message(author = owner, check=lambda message: message.server is None)
                    if welcomemsgreply.content.lower() == "n":
                        server_dict_temp['welcomemsg'] = "default"
                        await Squirtudo.send_message(owner, embed=discord.Embed(colour=discord.Colour.green(), description="Default welcome message set"))
                        break
                        ...
                        await Squirtudo.send_message(owner, embed=discord.Embed(colour=discord.Colour.lighter_grey(), description="Please shorten your message to less than 500 characters."))
                        continue
                    else:
                        ...
                        if errors:
                            if welcomemessage.startswith("[") and welcomemessage.endswith("]"):
                                ...
                                await Squirtudo.send_message(owner, embed=embed)
                            else:
                                await Squirtudo.send_message(owner, "{msg}\n\n**Warning:**\nThe following could not be found: {errors}".format(msg=welcomemessage, errors=', '.join(errors)))
                            await Squirtudo.send_message(owner, embed=discord.Embed(colour=discord.Colour.lighter_grey(), description="Please check the data given and retry a new welcome message, or reply with **N** to use the default."))
                            continue
                        else:
                            if welcomemessage.startswith("[") and welcomemessage.endswith("]"):
                                embed = discord.Embed(colour=server.me.colour, description=welcomemessage[1:-1].format(user=owner.mention))
                        ...
                        if res == '❎':
                            await Squirtudo.send_message(owner, embed=discord.Embed(colour=discord.Colour.lighter_grey(), description="Please enter a new welcome message, or reply with **N** to use the default."))
                            continue
                        else:
                            server_dict_temp['welcomemsg'] = welcomemessage
                            await Squirtudo.send_message(owner, embed=discord.Embed(colour=discord.Colour.green(), description="Welcome Message set to:\n\n{}".format(server_dict_temp['welcomemsg'])))
                            break
                    break
...
async def setstatus(ctx, member: discord.Member, status,*, status_counts: str = ''):
    ...
    Only usable by admins."""
    valid_status_list = ['interested', 'i', 'maybe', 'coming', 'c', 'here', 'h', 'cancel','x', 'o']
    if status not in valid_status_list:
        await Squirtudo.send_message(ctx.message.channel, "{status} is not a valid status!".format(status=status))
        return
...
async def about(ctx):
    ...
    about = (
        "I'm A Pokemon Go helper bot for Discord!\n\n"
...
async def team(ctx):
    ...
    for team in config['team_dict'].keys():
        if team.lower() not in lowercase_roles:
            try:
                temp_role = await Squirtudo.create_role(server, name=team.lower(),hoist = False, mentionable = True)
                server_roles.append(team.lower())
            except discord.errors.HTTPException:
                await Squirtudo.send_message(message.channel, "Maximum guild roles reached.")
                return
    ...
    for team in server_roles:
        ...
        if temp_role:
            ...
            if role and role.name.lower() == "harmony" and harmony in ctx.message.author.roles:
                # then report that a role is already assigned
                await Squirtudo.send_message(ctx.message.channel, _("You are already in Team Harmony!"))
                return
    ...
    # Check if the role is configured on the server
        ...
        try:
            if harmony and harmony in ctx.message.author.roles:
                await Squirtudo.remove_roles(ctx.message.author, harmony)
            await Squirtudo.add_roles(ctx.message.author, role)
            await Squirtudo.send_message(ctx.message.channel, _("Added {member} to Team {team_name}! {team_emoji}").format(member=ctx.message.author.mention, team_name=role.name.capitalize(), team_emoji=parse_emoji(ctx.message.server, config['team_dict'][entered_team])))
...
async def want(ctx):
    ...
    for want in want_list:
        ...
        if role is None:
            try:
                ...
                await Squirtudo.send_message(message.channel, "Maximum guild roles reached. Pokemon not added.")
                return
    ...
    if len(want_list) == 1 and (len(added_list) == 1 or len(spellcheck_dict) == 1 or len(already_want_list) == 1):
        if len(added_list) == 1:
            ...
            await Squirtudo.send_message(channel, content=_("Got it! {member} wants {pokemon}").format(member=ctx.message.author.mention, pokemon=added_list[0].capitalize()),embed=want_embed)
            return
...
async def _wild(message):
    ...
    if len(wild_split) <= 1:
        ...
        wild_img_url = "https://raw.githubusercontent.com/FoglyOgly/Squirtudo/master/images/pkmn/{0}_.png?cache=3".format(str(wild_number).zfill(3))
        ...
        wildreportmsg = await Squirtudo.send_message(message.channel, content=_("Wild {pokemon} reported by {member}! Details: {location_details}").format(pokemon=wild, member=message.author.mention, location_details=wild_details),embed=wild_embed)
...
async def _raid(message):
    ...
    if not raid_match:
        ...
        await Squirtudo.send_message(message.channel, "The Pokemon {pokemon} only appears in EX Raids! Use **!exraid** to report one!".format(pokemon=entered_raid.capitalize()))
        return

    if raidexp is not False:
        if _timercheck(raidexp, raid_info["raid_eggs"][get_level(entered_raid)]['raidtime']):
            await Squirtudo.send_message(message.channel, _("Squirtudo...that's too long. Level {raidlevel} raids currently last no more than {raidtime} minutes...").format(raidlevel=get_level(entered_raid), raidtime=raid_info["raid_eggs"][get_level(entered_raid)]['raidtime']))
            return
    ...
    raidreport = await Squirtudo.send_message(message.channel, content = _("{pokemon} raid reported by {member}! Details: {location_details}. Coordinate in {raid_channel}").format(pokemon=entered_raid.capitalize(), member=message.author.mention, location_details=raid_details, raid_channel=raid_channel.mention),embed=raid_embed)
...
async def _raidegg(message):
    ...
    if raidexp is not False:
        if _timercheck(raidexp, raid_info["raid_eggs"][str(egg_level)]['hatchtime']):
            await Squirtudo.send_message(message.channel, _("Squirtudo...that's too long. Level {raidlevel} Raid Eggs currently last no more than {hatchtime} minutes...").format(raidlevel=egg_level,hatchtime=raid_info["raid_eggs"][str(egg_level)]['hatchtime']))
            return
    ...
    if egg_level > 5 or egg_level == 0:
        ...
        if message.author.avatar:
            ...
            raid_embed.set_footer(text=_("Reported by @{author} - {timestamp}").format(author=message.author.display_name, timestamp=timestamp), icon_url=message.author.default_avatar_url)
        raid_embed.set_thumbnail(url=raid_img_url)
        raidreport = await Squirtudo.send_message(message.channel, content = _("Level {level} raid egg reported by {member}! Details: {location_details}. Coordinate in {raid_channel}").format(level=egg_level, member=message.author.mention, location_details=raid_details, raid_channel=raid_channel.mention),embed=raid_embed)
        await asyncio.sleep(1) #Wait for the channel to be created.
...
async def _eggassume(args, raid_channel):
    ...
    if raidrole is None:
        try:
            ...
            await Squirtudo.send_message(raid_channel, "Maximum guild roles reached.")
    ...
    try:
        getraid_message = await Squirtudo.edit_message(raid_message, new_content=raid_message.content, embed=raid_embed)
        raid_message = getraid_message.id
    except discord.errors.NotFound:
        raid_message = None
    try:
        getegg_report = await Squirtudo.edit_message(egg_report, new_content=egg_report.content, embed=raid_embed)
        egg_report = getegg_report.id
    except discord.errors.NotFound:
        egg_report = None
    await Squirtudo.send_message(raid_channel, _("This egg will be assumed to be {pokemon} when it hatches!").format(pokemon=raidrole))
...
This channel will be deleted five minutes after the timer expires.""").format(pokemon=entered_raid.capitalize(), member=raid_messageauthor.mention, citychannel=reportcitychannel.mention, location_details=egg_address,tutorial=Squirtudo.get_channel('406875985938743296').mention)
    ...
    if not raid_match:
        ...
        if get_number(entered_raid) not in raid_info['raid_eggs'][egglevel]['pokemon']:
            await Squirtudo.send_message(raid_channel, _("The Pokemon {pokemon} does not hatch from level {level} raid eggs!").format(pokemon=entered_raid.capitalize(), level=egglevel))
            return
    if raid_channel.name.startswith("level") or raid_channel.name.startswith("ex-raid"):
        raid_channel_name = entered_raid + "-" + sanitize_channel_name(egg_address)
    elif raid_channel.name.startswith("hatched-level"):
        raid_channel_name = entered_raid + "-" +sanitize_channel_name(egg_address)
    elif raid_channel.name.startswith("hatched"):
        raid_channel_name = raid_channel.name[8:15] + entered_raid + "-" +sanitize_channel_name(egg_address)
    else: 
        raid_channel_name = raid_channel.name[:7] + entered_raid + "-" + sanitize_channel_name(egg_address)
    oldembed = raid_message.embeds[0]
    raid_gmaps_link = oldembed['url']
    ...
    for field in oldembed['fields']:
        if "team" in field['name'].lower() or "status" in field['name'].lower():
            raid_embed.add_field(name=field['name'], value=field['value'], inline=field['inline'])
    try:
        getraid_message = await Squirtudo.edit_message(raid_message, new_content=raidmsg, embed=raid_embed)
        raid_message = getraid_message.id
    except (discord.errors.NotFound, AttributeError):
        raid_message = None
    try:
        getegg_report = await Squirtudo.edit_message(egg_report, new_content=raidreportcontent, embed=raid_embed)
        egg_report = getegg_report.id
    except (discord.errors.NotFound, AttributeError):
        egg_report = None
...
This channel will be deleted five minutes after the timer expires.""").format(member=message.author.mention, citychannel=message.channel.mention, location_details=raid_details,tutorial=Squirtudo.get_channel('406875985938743296').mention)
    ...
    await Squirtudo.send_message(raid_channel, content = _("Hey {member}, if you can, set the time left until the egg hatches using **!timerset <date and time>** so others can check it with **!timer**. **<date and time>** should look like it does on your EX Raid pass.").format(member=message.author.mention))
    event_loop.create_task(expiry_check(raid_channel))
...
async def _invite(ctx):
    ...
    if exraidcount == 0:
        await bot.send_message(channel, "No EX Raids have been reported in this server! Use **!exraid** to report one!")
        return
    await bot.send_message(channel, "{0}, the following {1} EX Raids have been reported:\n{2}\nReply with **the number** (1, 2, etc) of the EX Raid you have been invited to. If none of them match your invite, type 'N' and report it with **!exraid**".format(author.mention, str(exraidcount), exraidlist))
    reply = await bot.wait_for_message(author=author)
    if reply.content.lower() == 'n':
        await bot.send_message(channel, "Be sure to report your EX Raid with **!exraid**!")
    elif not reply.content.isdigit() or int(reply.content) > exraidcount:
        await bot.send_message(channel, "I couldn't tell which EX Raid you meant! Try the **!invite** command again, and make sure you respond with the number of the channel that matches!")
        ...
        await bot.send_message(channel, "{0}, you can now send messages in {1}! Make sure you let the trainers in there know if you can make it to the EX Raid!".format(author.mention, exraid_channel.mention))
        await _maybe(exraid_channel, author, 1, party=None)
    else:
        await bot.send_message(channel, "I couldn't understand your reply! Try the **!invite** command again!")
...
async def _timerset(raidchannel, exptime):
    ...
    if server_dict[server.id]['raidchannel_dict'][raidchannel.id]['type'] == "egg":
        ...
        topicstr += _("Ends on {end}").format(end=end.strftime("%B %d at %I:%M %p (%H:%M)"))
    await Squirtudo.edit_channel(raidchannel, topic=topicstr, name=new_name)
...
@Squirtudo.command(pass_context=True,aliases=["st"])
@checks.activeraidchannel()
...
@Squirtudo.command(pass_context=True,aliases=["d"])
@checks.activeraidchannel()
...
async def _coming(channel, author, count, party):
    ...
    if not party:
        ...
        party = [allblue, allred, allyellow, allunknown]
    if count == 1:
        await Squirtudo.send_message(channel, _("{member} is on the way!").format(member=author.display_name))
    else:
        await Squirtudo.send_message(channel, _("{member} is on the way with a total of {trainer_count} trainers! {blue_emoji}: {mystic} | {red_emoji}: {valor} | {yellow_emoji}: {instinct} | :grey_question:: {unknown}").format(member=author.display_name, trainer_count=count, blue_emoji=parse_emoji(channel.server, config['team_dict']['mystic']),mystic=party[0],red_emoji=parse_emoji(channel.server, config['team_dict']['valor']),valor=party[1], instinct=party[2], yellow_emoji=parse_emoji(channel.server,config['team_dict']['instinct']), unknown=party[3]))
...
async def _here(channel, author, count, party):
    ...
    if not party:
        ...
        party = [allblue, allred, allyellow, allunknown]
    if count == 1:
        await Squirtudo.send_message(channel, _("{member} is at the raid!"+lobbymsg).format(member=author.display_name))
    else:
        await Squirtudo.send_message(channel, _("{member} is at the raid with a total of {trainer_count} trainers! {blue_emoji}: {mystic} | {red_emoji}: {valor} | {yellow_emoji}: {instinct} | :grey_question:: {unknown}"+lobbymsg).format(member=author.display_name, trainer_count=count, blue_emoji=parse_emoji(channel.server, config['team_dict']['mystic']),mystic=party[0],red_emoji=parse_emoji(channel.server, config['team_dict']['valor']),valor=party[1], instinct=party[2], yellow_emoji=parse_emoji(channel.server,config['team_dict']['instinct']), unknown=party[3]))
    # Add trainer name to trainer list
    if author.id not in trainer_dict:
        trainer_dict[author.id] = {}
...
async def _party_status(ctx, total, teamcounts):
    ...
    for count in teamcounts:
        if count.isdigit():
            if total:
                return await Squirtudo.send_message(
    ...
    if total:
        if int(team_total) > int(total):
            return await Squirtudo.send_message(channel,
        ...
        if int(total) > int(team_total):
            if team_aliases[my_team][1]:
                if unknown[1]:
                    return await Squirtudo.send_message(channel, "Something is not adding up! Try making sure your total matches what each team adds up to!")
...
async def _edit_party(channel, author):
    ...
    reportchannel = Squirtudo.get_channel(server_dict[channel.server.id]['raidchannel_dict'][channel.id]['reportcity'])
    try:
        reportmsg = await Squirtudo.get_message(reportchannel, server_dict[channel.server.id]['raidchannel_dict'][channel.id]['raidreport'])
    except:
        pass
    raidmsg = await Squirtudo.get_message(channel, server_dict[channel.server.id]['raidchannel_dict'][channel.id]['raidmessage'])
    ...
    try:
        await Squirtudo.edit_message(reportmsg, embed=newembed)
    except:
        pass
    try:
        await Squirtudo.edit_message(raidmsg, embed=newembed)
    except:
        pass
...
async def _lobby(message, count):
    ...
    if count == 1:
        ...
        await Squirtudo.send_message(message.channel, _("{member} is entering the lobby with a total of {trainer_count} trainers!").format(member=message.author.display_name, trainer_count=count))
    # Add trainer name to trainer list
...
@Squirtudo.command(pass_context=True,aliases=["x","o","out"])
@checks.activeraidchannel()
...
async def _cancel(channel, author):
    ...
    if t_dict['status'] == "maybe":
        if t_dict['count'] == 1:
            ...
            await Squirtudo.send_message(channel, _("{member} and their total of {trainer_count} trainers are no longer interested!").format(member=author.display_name, trainer_count=t_dict['count']))
    if t_dict['status'] == "waiting":
        if t_dict['count'] == 1:
            ...
            await Squirtudo.send_message(channel, _("{member} and their total of {trainer_count} trainers have left the raid!").format(member=author.display_name, trainer_count=t_dict['count']))
    if t_dict['status'] == "omw":
        if t_dict['count'] == 1:
            await Squirtudo.send_message(channel, _("{member} is no longer on their way!").format(member=author.display_name))
        else:
            await Squirtudo.send_message(channel, _("{member} and their total of {trainer_count} trainers are no longer on their way!").format(member=author.display_name, trainer_count=t_dict['count']))
    if t_dict['status'] == "lobby":
        if t_dict['count'] == 1:
            ...
            await Squirtudo.send_message(channel, "{member} and their total of {trainer_count} trainers have backed out of the lobby!".format(member=author.display_name, trainer_count=t_dict['count']))
    t_dict['status'] = None
        ...
        if res:
            for trainer in trainer_list:
                if trainer in trainer_dict:
                    trainer_dict[trainer]['status'] = "waiting"
            await Squirtudo.send_message(channel, "{author}, {user} confirmed the group is backing out!".format(author=author.mention,user=res.user.mention))
            try:
                del server_dict[server.id]['raidchannel_dict'][channel.id]['lobby']
            except KeyError:
                pass
...
@Squirtudo.group(pass_context=True,aliases=["lists","l","ls"])
async def list(ctx):
...
@Squirtudo.command(pass_context=True,aliases=["a","si"])
@checks.activeraidchannel()
async def add(ctx, *, extras: str = None):
    ...
    await _add(ctx, extras_list, unknown_list)

async def _add(ctx, extras_list, unknown_list):
    if extras_list:
        extras_mention = []
        extras_name = []
        for extras in extras_list:
            ...
            ctx.message.content ="{}{} {}".format(ctx.prefix, 'i', '')
            await ctx.bot.process_commands(ctx.message)

@Squirtudo.command(pass_context=True,aliases=["sh"])
@checks.activeraidchannel()
async def sethere(ctx, *, extras: str = None):
    """Set trainers as here.

    Usage: !sethere <extras> // sets <extras> status to *here* -- those users can then update their status accordingly.
    (works with or without comma separation, but assumes a space corresponds to a new user)
    Works only in raid channels. Works only with existing discord users who are members in the server."""
    trainer_dict = server_dict[ctx.message.server.id]['raidchannel_dict'][ctx.message.channel.id]['trainer_dict']
    if extras is not None:
    ...
    await _sethere(ctx, extras_list, unknown_list)

async def _sethere(ctx, extras_list, unknown_list):
    if extras_list:
        extras_mention = []
        extras_name = []
        for extras in extras_list:
            ...
            extras_mention.append(user.display_name)
            ctx.message.author = user
            ctx.message.content ="{}{} {}".format(ctx.prefix, 'h', '')
            await ctx.bot.process_commands(ctx.message)
...
async def setcoming(ctx, *, extras: str = None):
    ...
    await _setcoming(ctx, extras_list, unknown_list)

async def _setcoming(ctx, extras_list, unknown_list):
    if extras_list:
        extras_mention = []
        extras_name = []
        for extras in extras_list:
    ...
    for trainer in trainer_dict.keys():
        ...
        if trainer_dict[trainer]['status']=='maybe' and memberexists:
            ...
            if trainer_dict[trainer]['count'] > 1:
                name_list.append("**"+user.display_name+"(+"+str(trainer_dict[trainer]['count']-1)+")**")
            else: name_list.append("**"+user.display_name+"**")
            maybe_list.append(user.mention)
    if ctx_maybecount > 0:
